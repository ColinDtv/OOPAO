...
for i_g in range(3):
    ...
    for i in range(N):
        ...
        print(np.std(1e9*phase_turb[tel.pupilLogical]))
        ...
        print(np.std(1e9*phase_wfs[tel.pupilLogical]))
    
        mod_coeffs_res[i,:,i_g] = projector@phase_wfs
        
    # out_CL.append([mod_coeffs_res,mod_coeffs_turb])
...
plt.loglog(1e9*np.std(mod_coeffs_res[start:end,:,2],axis = 0),label = 'Loop Gain 1')
...
# remove mean value
mod_coeffs_turb_mean_rem = mod_coeffs_turb[start:,:] - np.mean(mod_coeffs_turb[start:end,:], axis = 0)
mod_coeffs_res_mean_rem = mod_coeffs_res[start:,:] - np.mean(mod_coeffs_res[start:end,:], axis = 0)
plt.legend(['Turbulence','Residuals'])
...
for i_g in range(3):
    ...
    for i_mode in range(3):
        ...
        f, Poi = signal.csd(out[mode,start:end],inp[mode,start:end],fs,nperseg=4*128)
        f, Pii = signal.csd(inp[mode,start:end],inp[mode,start:end],fs,nperseg=4*128)
    ...
    #     plt.figure(11)
    #     plt.semilogx(f,(DSP),label = 'KL Mode '+str(mod_test[i_mode]+1))
    #     plt.title('Wind-Speed = 10 m/s')
    #     plt.grid(which='both')
    #     plt.xlabel('Frequency [Hz]')
...
from scipy import signal 
out = mod_coeffs_res.T
inp = mod_coeffs_turb.T
...
for i_mode in range(2):
    mode = mod_test[i_mode]    
    f, Poi = signal.csd(out[mode,start:end],inp[mode,start:end],fs,nperseg=4*256)
    f, Pii = signal.csd(inp[mode,start:end],inp[mode,start:end],fs,nperseg=4*256)
    ...
    plt.figure(9)
    plt.subplot(1,2,i_mode+1)
    plt.plot(np.abs(Her)/np.abs(Poi[1:]/Pii[1:]))
    plt.title('KL Mode ' + str(mod_test[i_mode]))
#    plt.grid(which='both')
    plt.xlabel('Frequency [Hz]')
    plt.ylabel(r'20 log($\frac{\Phi_{res}}{\Phi_{turb}}$)')
#    plt.ylim([-35,10])
    makeSquareAxes(plt.gca())
    
    plt.figure(10)
    plt.subplot(1,2,i_mode+1)
    plt.semilogx(f[1:],Her_dB)
    plt.semilogx(f,(FTR))
    plt.title('KL Mode ' + str(mod_test[i_mode]))
    plt.grid(which='both')
    plt.xlabel('Frequency [Hz]')
...
plt.plot(f[1:],Her_dB*0,'k')

#%%

fs = 1/tel.samplingTime

x = np.linspace(1,N,N)*tel.samplingTime

from scipy import signal

f, ps_turb = signal.welch(mod_coeffs_turb.T,fs,nperseg = 256*3)

f, ps_res = signal.welch(mod_coeffs_res.T,fs,nperseg = 256*3)


mode = 1

FTR = ps_res[mode,:]/ps_turb[mode,:]
